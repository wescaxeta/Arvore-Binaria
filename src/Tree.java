...
class Tree {
	  ...
	  public void inserir(Pessoa v) {
	    ...
	    if (root == null) root = novo;
	      ...
	      while(true) {
	        anterior = atual;
	        if (v.nome.compareTo(atual.item.nome + " ") < 0 || v.nome.compareTo(atual.item.nome + " ") == 0) { // ir para esquerda
	          atual = atual.esq;
	          if (atual == null) {
	            anterior.esq = novo;
	            return;
	  ...
	  public No buscar(String nome) {
	    ...
	    while (!atual.item.nome.equals(nome)) { // enquanto nao encontrou
	      if(nome.compareTo(atual.item.nome) < 0) atual = atual.esq; // caminha para esquerda
	  ...
	  public boolean remover(String nome) {
	    ...
	    while (!atual.item.nome.equals(nome)) { // enquanto nao encontrou
	      pai = atual;
	      if(nome.compareTo(atual.item.nome) > 0) { // caminha para esquerda
	        atual = atual.esq;
	  ...
	  public void inOrder(No atual) {
	    if (atual != null) {
	      inOrder(atual.esq);
	      System.out.print(atual.item.nome + " ");
	      inOrder(atual.dir);
	    }
	  }
